...

{
    ...
    {
        // dd($request->all(), $bank->toArray());
        try {
            DB::beginTransaction();
            $order = new transaksi();
            $customer = User::where('karyawan_id', Auth::user()->id)->where('id', $request->customer_id)->first();
            $bank = DataBank::first();
            ...
            if ($order) {
                ...
                if (setNotificationWhatsappOrderSelesai(1) == 1 && getTokenWhatsapp() != null) {
                    $waCustomer = $customer->no_telp; // get nomor whatsapp customer
                    $nameCustomer = $customer->name; // get name customer

                    // Inisialisasi pesan dengan nilai default
                    $message = "Halo Kak {$nameCustomer},\n\n" .
                        "Detail Transaksi Laundry77 {$nama_laundry} :\n" .
                        "Invoice: {$order->invoice}\n" .
                        "Customer: {$order->customer}\n" .
                        "Tanggal Transaksi: {$order->tgl_transaksi}\n\n" .
                        "Jenis Layanan: {$jenisPakaian->jenis}\n" .
                        "Berat: {$order->kg} kg\n\n" .
                        ($order->disc ? "Diskon: {$order->disc}%\n" : '') .
                        "Harga per kg: Rp " . number_format($order->harga, 2) . "\n" .
                        "Total Harga: Rp " . number_format($order->harga_akhir, 2) . "\n\n" .
    ...
    public function updateStatusLaundry(Request $request)
    {
        ...
        if ($transaksi->status_payment == 'Pending') {
            ...
            if ($transaksi->status_order == 'Process') {
                ...
                $body       = 'Pakaian Sudah Selesai dan Sudah Bisa Diambil, Terima Kasih Sudah Menggunakan Layanan Kami';
                $kategori   = 'info';
                ...
                if (setNotificationWhatsappOrderSelesai(1) == 1 && getTokenWhatsapp() != null) {
                    ...
                    notificationWhatsapp(
                        getTokenWhatsapp(), // Token
                        $waCustomer, // nomor whatsapp
                        'Halo Kak ' . $nameCustomer . ' Laundry kamu sudah selesai dan sudah bisa diambil. Terima Kasih sudah menggunakan layanan kami. ' // pesan
                    );
                }
