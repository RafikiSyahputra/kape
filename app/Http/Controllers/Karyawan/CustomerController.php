...
class CustomerController extends Controller
{
    ...
    public function store(AddCustomerRequest $request)
    {
        ...
        try {
            ...
            $phone_number = '62' . ltrim($request->no_telp, '0'); // Menambahkan '62' di depan nomor telepon
    ...
    public function sendBroadcast(Request $request)
    {
        ...
        try {
            ...
            $customers = User::where('auth', 'Customer')->pluck('no_telp')->toArray();
            // $customers = array_filter($customers, function ($cus) {
            //     return $cus !== "6285376623384";
            // });
            if($r['all']?? false) {
                broadcastWa($customers, $r['message']);

            } else {
                broadcastWa($r['customers'], $r['message']);
            }

            return back()->with('success', 'Broadcast WhatsApp berhasil dikirim.');
        } catch (\Exception $e) {
    ...
    {
        return view("karyawan.customer.broadcast");
    }
}
